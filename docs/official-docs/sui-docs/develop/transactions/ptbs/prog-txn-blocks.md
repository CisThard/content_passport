<!-- Source: https://docs.sui.io/develop/transactions/ptbs/prog-txn-blocks -->

* [](</>)
  * [Building Transactions](</develop/transactions/>)
  * [Programmable Transaction Blocks](</develop/transactions/ptbs/>)
  * What is a PTB?
...
The `Argument` enum has 4 variants:
  ...
  * `GasCoin`: References the SUI coin used to pay for gas. When using address balance gas payment (empty gas payment list), `GasCoin` references a synthetic coin created from the sender or sponsor address balance. **This is separate from other inputs because it has special restrictions** :

    * `&mut` or `&` usage anywhere.
    * Only by value through `TransferObjects` or `sui::coin::send_funds`
      * If you need an owned `Coin<SUI>` from the `GasCoin`, first use `SplitCoins`.
...
The most important consideration at this stage is the effects on the gas coin. At the beginning of execution, the system withdraws the maximum gas budget (in terms of SUI) from the gas coin. The system returns any unused gas to the gas coin at the end of execution, even if the coin has changed owners.
...
**Return types:** Move calls cannot return references (`&T` or `&mut T`).
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/develop/transactions/ptbs/prog-txn-blocks.mdx>)

[PreviousProgrammable Transaction Blocks](</develop/transactions/ptbs/>)[NextBuilding Transactions](</develop/transactions/ptbs/building-ptb>)
