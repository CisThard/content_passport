<!-- Source: https://docs.sui.io/develop/objects/transfers/transfer-policies -->
...
A rule needs to provide 4 main components:
  ...
  2. A `config` type which is stored in the `TransferPolicy` and is used to configure the rule.

  3. A set function which adds the rule to the `TransferPolicy`. The `TransferPolicyCap` holder must perform this action.
...
There are 2 ways to authorize an action:
  ...
  2. Dynamic, through a capability pattern.
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/develop/objects/transfers/transfer-policies.mdx>)

[PreviousCustom Transfer Rules](</develop/objects/transfers/custom-rules>)[NextTransfer to Object](</develop/objects/transfers/transfer-to-object>)
