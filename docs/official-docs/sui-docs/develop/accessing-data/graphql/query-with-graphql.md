...
[code]
    Convert this Sui JSON-RPC query to GraphQL. Include the query, variables, pagination handling, and TypeScript caller code.
[/code]
...
If there are too many transactions to return in a single response, the service applies a limit on the maximum page size for variable size responses (like the `transactionBlock` query) and you must fetch further results through [pagination](<https://graphql.org/learn/pagination/>).
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/develop/accessing-data/graphql/query-with-graphql.mdx>)

[PreviousGraphQL for Sui RPC](</develop/accessing-data/graphql/graphql-rpc>)[NextArchival Store and Service](</develop/accessing-data/archival-store/>)
