<!-- Source: https://docs.sui.io/develop/manage-packages/move-package-management -->

* [](</>)
  * [Managing Packages](</develop/manage-packages/>)
...
Only include an environment in your manifest if you expect other packages to link against your package on that network. You usually do not want to include local networks in your manifest, because they are typically short-lived and private. Instead, you should consider using the `test-publish` command to publish your package and its dependencies on a local network. The `test-publish` command gives you much more flexibility to manage your deployment.
...
The command `sui client test-publish --pubfile-path <pubfile> --build-env <env>` builds the root package for the `<env>` environment (see the build environment) and then publishes it to a chain using the addresses from the ephemeral publication file `<pubfile>`.
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/develop/manage-packages/move-package-management.mdx>)

[PreviousManaging Packages](</develop/manage-packages/>)[NextAddress Management (Legacy)](</develop/manage-packages/automated-address-management>)
