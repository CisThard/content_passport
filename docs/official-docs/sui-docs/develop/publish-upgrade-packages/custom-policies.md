<!-- Source: https://docs.sui.io/develop/publish-upgrade-packages/custom-policies -->
...
[code]
    Create an operator runbook for package upgrades and admin actions: required caps, signer/custody steps, exact commands, verification, and rollback/communication plan.
[/code]
...
Package upgrades must occur end-to-end in a single transaction and are composed of 3 commands:
  ...
  2. **Execution:** Consume the `UpgradeTicket` and verify the package bytecode and compatibility against the previous version, then create the onchain object representing the upgraded package. Return an `UpgradeReceipt` as a result on success.
...
[crates/sui-framework/packages/sui-framework/sources/package.move](<https://github.com/MystenLabs/sui/blob/main/crates/sui-framework/packages/sui-framework/sources/package.move>)
[code]
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/develop/publish-upgrade-packages/custom-policies.mdx>)

[PreviousUpgrading](</develop/publish-upgrade-packages/upgrade>)[NextVersioning](</develop/publish-upgrade-packages/versioning>)
