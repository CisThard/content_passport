<!-- Source: https://docs.sui.io/references/package-managers/package-manager-migration -->
...
Although the new package system will continue to work with existing packages, it is encouraged to migrate to the new format as soon as possible. The steps to migrate are:
  ...
  1. Replace the `name` field in `Move.toml` with the name that your Move code uses for your package (more info).
  2. Replace your dependency names in `Move.toml` to match the names your Move code uses to refer to them (more info).
  3. Remove the `[addresses]` and `[dev-addresses]` sections of `Move.toml` (more info).
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/references/package-managers/package-manager-migration.mdx>)

[NextManifest Reference](</references/package-managers/manifest-reference>)
