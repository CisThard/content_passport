<!-- Source: https://docs.sui.io/onchain-finance/payment-intents -->
...
A payment intent moves through three phases:
  ...
  2. **Plan.** The app arranges the resolved values into an ordered sequence of PTB commands. Order matters because later commands can consume outputs from earlier ones.

  3. **Execute.** The user signs once. The payment intent submits to the network and either commits all commands or reverts all of them.
...
The planner orders the operations so that later math uses the correct inputs:
  ...
  2. Swap 900 USDC to USDsui and deposit into the NAVI pool. This is 50% of the 1,800 USDC that remains after step 1, not 50% of the original 2,000.

  3. Transfer 100 USDC to `0xa4bb…b27c`.
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/onchain-finance/payment-intents.mdx>)

[PreviousPayment Kit](</onchain-finance/payment-kit>)
