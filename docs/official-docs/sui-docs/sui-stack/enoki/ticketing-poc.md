<!-- Source: https://docs.sui.io/sui-stack/enoki/ticketing-poc -->
...
Alternatively, you can use the [Move web IDE](<https://www.playmove.dev/>), which does not require a download. It does not support all functions necessary for this guide, however.
  ...
  * A [Google OAuth client](<https://console.cloud.google.com/>) and its client ID. Your Google OAuth client must have `http://localhost:3000/` set as an authorized JavaScript origin and authorized redirect URI.

  * An [Enoki](<https://portal.enoki.mystenlabs.com/>) app with:
...
![Enoki keys, including private and public](/assets/images/enoki_keys-5c25275dfa0bbcbc6fcbfcf6ee78bbc3.png)
...
File not found in manifest: `move/sources/ticket.move`. You probably need to run `pnpm prebuild` and restart the site.
...
File not found in manifest: `move/sources/ticket_stage.move`. You probably need to run `pnpm prebuild` and restart the site.
...
File not found in manifest: `move/sources/key_registry.move`. You probably need to run `pnpm prebuild` and restart the site.
...
File not found in manifest: `app/src/app/hooks/useMintTicket.tsx`. You probably need to run `pnpm prebuild` and restart the site.
...
File not found in manifest: `app/src/app/api/permit/mint-ticket/route.ts`. You probably need to run `pnpm prebuild` and restart the site.
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/sui-stack/enoki/ticketing-poc.mdx>)

[PreviousSolitaire](</sui-stack/enoki/solitaire>)[NextNautilus](</sui-stack/nautilus/>)
