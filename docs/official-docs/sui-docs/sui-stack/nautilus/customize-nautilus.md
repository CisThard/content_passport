<!-- Source: https://docs.sui.io/sui-stack/nautilus/customize-nautilus -->
...
The Nautilus server logic is located in the directory [`src/nautilus-server`](<https://github.com/MystenLabs/nautilus/tree/main/src/nautilus-server>). To customize the application, refer to the [Using Nautilus guide](</sui-stack/nautilus/using-nautilus>) as a template.

info

These customization, testing, and management steps are for self-managed TEEs. Alternatively, you can use the [Marlin Oyster marketplace](</sui-stack/nautilus/community-dev-tools>) for Docker-based simplicity that removes testing and management overhead.
...
Write frontend code to interact with the enclave for computation, and then send the resulting data to the Move contract for use. For the [weather example](</sui-stack/nautilus/using-nautilus>), you can request the enclave to retrieve weather data for a specific location:
[code] 
...
#### Where is the root of trust of AWS?​
  ...
  * It is stored as part of the Sui framework and used to verify AWS attestation documents. You can verify its hash by following the steps outlined on [AWS](<https://docs.aws.amazon.com/enclaves/latest/user/verify-root#validation-process>).
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/sui-stack/nautilus/customize-nautilus.mdx>)

[PreviousUsing Nautilus](</sui-stack/nautilus/using-nautilus>)[NextWeather Oracle](</sui-stack/nautilus/nautilus-weather-oracle>)
