<!-- Source: https://docs.sui.io/getting-started/onboarding/local-network -->
...
Localnet is a single-machine network you run yourself. Devnet is a shared public test network that Mysten Labs operates. These are different networks with separate state.
...
Verify the faucet is running (it starts with `--with-faucet`). Request tokens manually:
[code] 
...
If you see this error when re-publishing, remove the environment entry from `Published.toml` and re-run the publish command. For Localnet development, use `sui client test-publish` to avoid recording addresses that change after every genesis reset.

### Single validator is expected​

Localnet runs with a single validator by default. This is expected and sufficient for development.

[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/getting-started/onboarding/local-network.mdx>)

[PreviousInstall from Source](</getting-started/onboarding/install-source>)[NextConfigure a Sui Client](</getting-started/onboarding/configure-sui-client>)
