<!-- Source: https://docs.sui.io/operators/snapshots -->
...
To restore from a RocksDB snapshot, follow these steps:
  ...
  2. Download the snapshot to a temporary directory using cloud object store tooling. For example, if the `db-path` value points to `/opt/sui/db/authorities_db/full_node_db` and you want to restore from epoch 10, use one of these commands:

Use the AWS CLI for S3 buckets (provided you have credentials to associate with the download): `aws s3 cp s3://<BUCKET_NAME>/epoch_10 /tmp/epoch_10 --recursive --request-payer requester`.

Use `gsutil` for GCS buckets (provided you have a billing project for Requester Pays): `gsutil -u <BILLING_PROJECT> -m cp -r gs://<BUCKET_NAME>/epoch_10 /tmp/epoch_10`.

  3. Place the downloaded `epoch_[NUM]` directory at `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/sui/db/authorities_db/full_node_db/live`.
...
To restore using a formal snapshot, use the `sui-tool` binary. You can download `sui-tool` along with other `sui` binaries. See [Install Sui](</getting-started/onboarding/sui-install>) for more details.
...
To restore a node from a formal snapshot, complete the following steps:
  ...
  2. Run the command:
[code] sui-tool download-formal-snapshot --latest --genesis "<PATH-TO-GENESIS-BLOB>" \  
...
[/code]
     ...
     * `--epoch`: The epoch that you want to download. Mysten Labs hosted buckets keep only the last 90 epochs. You can check the most recent epoch on Sui explorers like [suivision](<https://suivision.xyz/>) or [suiscan](<https://suiscan.xyz/>).
...
**High throughput, Requester Pays enabled buckets:**
  ...
  * GCS and S3 buckets are set up with requester pays. You need to provide a set of valid AWS or GCP credentials when downloading from these buckets. [Requester Pays](<https://cloud.google.com/storage/docs/requester-pays>) means you are charged for the egress costs of pulling the snapshot data.
...
**Free, permissionless buckets:**
  ...
  * The free buckets are only available for formal snapshots. For example, `sui-tool download-formal-snapshot --no-sign-request` uses the free formal snapshot bucket.
...
![Mysten Managed Snapshots](/assets/images/managed-snapshots_opreator-guides_v1-3d28d2bce1812b1effe5a0bbcc70d20d.png)
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/operators/snapshots.mdx>)

[PreviousObservability](</operators/observability>)[NextFull Nodes](</operators/full-node/>)
