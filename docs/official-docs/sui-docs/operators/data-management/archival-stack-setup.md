<!-- Source: https://docs.sui.io/operators/data-management/archival-stack-setup -->
...
The archival stack consists of three components:
  ...
  * A [Sui full node](</operators/full-node/sui-full-node>) with gRPC enabled. After the initial backfill, you can stream checkpoints from a full node instead of polling the bucket for lower latency. See Steady-state operation.
...
---|---|---  
`--config`| No| Path to TOML configuration file. If you omit this flag, the framework uses defaults. See Indexer configuration.  
...
The indexer accepts an optional TOML configuration file through `--config`. For tip-of-chain indexing, the framework defaults provide a good starting point and you can omit the flag entirely. You primarily need configuration tuning during backfill.
...
[backfill.toml](<https://github.com/MystenLabs/sui/blob/main/backfill.toml>)
[code]
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/operators/data-management/archival-stack-setup.mdx>)

[PreviousRunning a Remote Store](</operators/data-management/remote-store-setup>)[NextArchive Data](</operators/data-management/archives>)
