<!-- Source: https://docs.sui.io/operators/data-management/managing-data -->
...
Managing the data on your Sui full node is critical to ensuring a healthy Sui network. This topic provides a high-level description of data management on Sui full nodes that you can use to optimize your full node configuration. For more information about Sui full nodes, including pruning policies and archival settings, see [Run a Sui Full Node](</operators/full-node/sui-full-node>).

## Basic Sui full node functionality​

The minimal version of a Sui full node executes all the transactions Sui validators commit. Sui full nodes also orchestrate submitting new transactions to the system:

![Basic Sui full node functionality](/assets/images/basic-functionality_data-management_v1-a813646fa16e7987e3e560fd03d03d3b.png)
...
A Sui archive instance stores the full Sui transaction history since genesis in a database-agnostic format. This includes information about transactions (with client authentication), effects, events, and checkpoints. You can use archival storage for data auditing and for replaying historic transactions.

A full node that starts from scratch can replay (and thus re-verify) transactions that occurred since Sui genesis from the given archive by [configuring archival fallback](</operators/data-management/archives#set-up-archival-fallback>) in the `fullnode.yaml` configuration file to point to the cloud bucket that stores the archive.
...
If you prune transactions, archival nodes can help ensure lagging peer nodes don't lose any information. For more information, see [Sui Archives](</operators/data-management/archives>).
...
This configuration manages secondary indexing in addition to the latest state, but prunes historical data after a few epochs. A full node with this configuration:
  ...
  * Answers RPC queries that require historic transactions through a fallback to retrieve the data from a remote key-value store: `sui_getTransactionBlock()`.
...
[Edit this page](<https://github.com/MystenLabs/sui/tree/main/docs/docs/../content/operators/data-management/managing-data.mdx>)

[PreviousData Indexing and Archives](</operators/data-management/>)[NextGraphQL and General-Purpose Indexer](</operators/data-management/indexer-stack-setup>)
