<!-- Source: https://docs.wal.app/docs/operator-guide/publishers/operating-publisher -->
...
By default, 8 sub-wallets are created and funded. You can change this with the `--n-clients` argument (see Set sub-wallets and upload concurrency for details). For basic local testing, 1 or 2 sub-wallets are usually sufficient.

## Start a local daemonâ
...
If you run the publisher without a reverse proxy, open **port 9001** on your firewall. With a reverse proxy (such as the nginx caching setup), only **port 443** needs to be open.

## Download the client configurationâ
...
## Create and fund the publisher walletâ
...
##### Step 1: Generate a new wallet.â
[code] 
...
##### Step 2: Create a publisher-specific client configuration.â

This assumes you have already downloaded the client configuration to `/opt/walrus/config/client_config.yaml` (see Download the client configuration above). Create a copy for the publisher that references the new wallet:
[code] 
...
##### Step 3: Fund the wallet with SUI and WAL.â
...
## Run a publisher manually for testingâ
...
## Sample `systemd` configurationâ

This section assumes that you have already downloaded the client configuration and created and funded the publisher wallet.
...
## Set sub-wallets and upload concurrencyâ
...
To operate a high-performance publisher with higher concurrency, use the following options:
  ...
  * `--n-clients NUM`: Creates a number of separate wallets used to perform concurrent Sui chain operations. Increase this value to allow more parallel uploads. A higher number requires more SUI and WAL coins initially because the funds are distributed to more wallets (see Create and fund the publisher wallet).
...
## Manage SUI coins in sub-walletsâ
...
## Understand `Blob` object lifecycleâ
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/operator-guide/publishers/operating-publisher.mdx>)

[PreviousStorage Node FAQ](</docs/operator-guide/storage-nodes/storage-node-faq>)[NextUse the Authenticated Publisher](</docs/operator-guide/publishers/auth-publisher>)

  * Start a local daemon
  * Download the client configuration
  * Create and fund the publisher wallet
