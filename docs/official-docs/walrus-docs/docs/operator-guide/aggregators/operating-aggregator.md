<!-- Source: https://docs.wal.app/docs/operator-guide/aggregators/operating-aggregator -->
...
If you run the aggregator without a reverse proxy, open **port 9000** on your firewall. With a reverse proxy (such as the nginx caching setup), only **port 443** needs to be open.
...
[code]
    ...
    for chunk_file in "${chunk_files[@]}"; do  
      ...
      if ! walrus store "${walrus_args[@]}" "$chunk_file"; then  
        exit_code=$?  
        error "â walrus store failed with exit code: $exit_code"  
        note "failed to store entire file. please address issue above and try again."  
        exit $exit_code  
      fi  
    done  
      
    note "â all chunks stored successfully"  
    
[/code]

For more information about maximum blob sizes, run `walrus info`.

[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/operator-guide/aggregators/operating-aggregator.mdx>)

[PreviousMainnet Publisher Production Guide](</docs/operator-guide/publishers/mainnet-production-guide>)[NextOperate an Upload Relay](</docs/operator-guide/upload-relay>)
