<!-- Source: https://docs.wal.app/docs/operator-guide/storage-nodes/storage-node-migration -->
...
This guide covers migrating a Walrus storage node to new hardware. It assumes you run the node (and optionally an aggregator and publisher) on the same host.
  ...
  * New host set up following the [Storage Node Setup](</docs/operator-guide/storage-nodes/storage-node-setup>) guide (through the [TLS setup](</docs/operator-guide/storage-nodes/storage-node-setup#tls-setup>) and [systemd service](</docs/operator-guide/storage-nodes/storage-node-setup#systemd>) sections. Do **not** run `walrus-node setup` or `register` on the new host.)
...
##### Step 1: Log into the new host as the `walrus` user.â

##### Step 2: Set the environment variables for the old host.â
[code] 
...
##### Step 3: Verify you can SSH into the old host from the new host.â

##### Step 4: Transfer the `/opt/walrus` directory.â
[code] 
    rsync -avz --progress $OLD_USER@$OLD_HOST:/opt/walrus/ /opt/walrus  
    
[/code]

## Second transfer and migrationâ

##### Step 1: Stop all services on the old host.â
...
##### Step 2: Run a second `rsync` to synchronize changes.â
...
##### Step 5: Verify transferred data.â
...
##### Step 2: Verify the onchain key.â
[code] 
...
##### Step 3: Verify the event count.â
...
##### Step 4: (Optional) Test your aggregator.â
[code] 
    curl https://AGGREGATOR_ADDRESS/v1/blobs/BLOB_ID  
    
[/code]

##### Step 5: (Optional) Test your publisher.â
[code] 
    curl -X PUT https://PUBLISHER_ADDRESS/v1/blobs -d "test"  
    
[/code]

##### Step 6: Clean up the old host.â

Only after verifying everything works, clean up the old host by removing `/opt/walrus`.

[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/operator-guide/storage-nodes/storage-node-migration.mdx>)

[PreviousStorage Node Maintenance](</docs/operator-guide/storage-nodes/storage-node-maintenance>)[NextCommission and Governance](</docs/operator-guide/storage-nodes/commission-governance>)
