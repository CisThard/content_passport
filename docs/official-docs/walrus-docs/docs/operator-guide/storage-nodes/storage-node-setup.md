<!-- Source: https://docs.wal.app/docs/operator-guide/storage-nodes/storage-node-setup -->
...
If you deviate from the standard setup below (user, directories, ports), make sure to adjust later steps accordingly. Some Walrus and Sui configuration files include absolute paths, so moving files without adjusting those paths might cause issues.
  ...
  * A large disk partition for blob storage, mounted at `/opt/walrus/db`
  * A public IP address with a DNS name pointing to it
  * Complete initial system setup

Initial system setup instructions

##### Step 1: Create a `walrus` user and group.â
[code]
    sudo useradd -s /bin/bash walrus  
    
[/code]

##### Step 2: Create the directory structure.â
[code]
...
##### Step 3: Mount the storage partition.â
...
##### Step 4: Configure the firewall:â
...
##### Step 2: Set the `SERVER_NAME` environment variable.â
...
##### Step 3: Create the deploy hook script.â
...
##### Step 4: Set the correct permissions.â
[code] 
...
##### Step 6: Obtain the certificate.â
...
## Create the systemd service fileâ
...
## Download binaries and generate configurationâ
...
### Create the client configurationâ
[code] 
...
### Generate the node configurationâ
...
The `walrus-node setup` command generates the node configuration and a Sui wallet. Key options:
  ...
  * `--price-currency`: The currency unit for `--storage-price` and `--write-price`. Accepts `frost` (default) or `nanousd`. See Pricing currency: FROST and NanoUSD for the difference between the two and how stable pricing works.
...
### Pricing currency: FROST and NanoUSDâ
...
#### How the WAL price monitor worksâ
...
When you select `nanousd`, the node automatically starts a background WAL price monitor that:
  ...
  2. Takes the median of the successfully fetched prices to dampen outliers from any single source.
  3. Caches the most recent price and exposes it to the configuration synchronizer.
  4. Pushes a new vote onchain only when the converted FROST price differs from the current onchain price by more than a configurable threshold (default 10%). This avoids spamming the chain with small fluctuations.
...
## Register and start the nodeâ

##### Step 1: Fund the wallet.â
...
##### Step 3: Set up commission and governance authorization.â
...
##### Step 5: Verify the node is running.â
[code] 
...
##### Step 6: Check the health endpoint.â
...
##### Step 7: Verify the onchain key.â
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/operator-guide/storage-nodes/storage-node-setup.mdx>)

[PreviousOperating a Storage Node](</docs/operator-guide/storage-nodes>)[NextStorage Node Maintenance](</docs/operator-guide/storage-nodes/storage-node-maintenance>)
  ...
  * Download binaries and generate configuration
    ...
    * Pricing currency: FROST and NanoUSD
