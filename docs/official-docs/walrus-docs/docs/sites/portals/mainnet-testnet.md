<!-- Source: https://docs.wal.app/docs/sites/portals/mainnet-testnet -->

* [](</>)
  * [Walrus Portals](</docs/sites/portals/deploy-locally>)
  * Configure a Local Sites Portal
...
Environment variables override any value set in `portal-config.yaml`. You can use this to apply per-deployment overrides without modifying the file. See Environment variable overrides for details.
...
**Type:** `string` **Required:** Yes **Accepted values:** `mainnet`, `testnet`
[code] 
    network: mainnet  
...
A separate set of Sui full node RPC endpoints used exclusively for allowlisted sites. This field uses the same structure as `rpc_urls`. Omit this field when `enable_allowlist` is `false`.
...
Each entry uses the same structure as `rpc_urls`.
...
## Environment variable overridesâ
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/sites/portals/mainnet-testnet.mdx>)

[PreviousDeploy a Local Portal](</docs/sites/portals/deploy-locally>)[NextSetting a SuiNS Name](</docs/sites/custom-domains/setting-a-suins-name>)
