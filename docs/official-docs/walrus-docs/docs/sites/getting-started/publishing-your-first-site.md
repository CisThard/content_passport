<!-- Source: https://docs.wal.app/docs/sites/getting-started/publishing-your-first-site -->
...
[code] 
    ...
    âââ file.svg  
    âââ index.html  
    âââ Oi-Regular.ttf  
    âââ walrus.svg  
    âââ ws-resources.json  
...
[code] 
    ...
    â  wal.app only supports sites deployed on mainnet.  
        ...
        ð Setup instructions: https://docs.wal.app/docs/sites/portals/deploy-locally.html#running-the-portal-locally  
      
        ð¡ Tip: You may also bring your own domain (/docs/sites/custom-domains/bringing-your-own-domain)  
...
[code] 
    ...
    â  wal.app only supports sites deployed on mainnet.  
        ...
        ð Setup instructions: https://docs.wal.app/walrus-sites/portal.html#running-the-portal-locally  
...
## Using a web frameworkâ

If you are building a Walrus Site using a web framework (such as React, Node.js, or Django), ensure you point the `site-builder` to your site's build output folder when running the `deploy` command. `site-builder` should not point to the project root.

When you use `site-builder deploy`, every file in the directory you point it to is uploaded. Uploading your project root would include directories like `node_modules/`, which can contain tens of thousands of files and hundreds of megabytes of dependencies. This results in:
  ...
  * Unnecessary files on your site. Source files, config, and dependencies are not needed to serve your site to visitors.
...
### Example: Vite projectâ

A typical Vite project structure looks like this before building:
[code] 
    my-website/  
    âââ node_modules/       # dependencies: never deploy this  
    âââ public/             # static assets  
    âââ src/                # source files: never deploy this  
    âââ index.html  
    âââ package.json  
...
Running `npm run build` generates a `dist/` folder containing only the files needed to serve your site:
[code] 
    my-website/  
    âââ dist/               # deploy this folder only  
    â   âââ assets/  
    â   âââ index.html  
    âââ node_modules/       # never deploy this  
    âââ public/  
    âââ src/                # never deploy this  
    âââ index.html  
    âââ package.json  
    âââ vite.config.js  
    âââ .gitignore  
...
Build folder names vary by framework Different frameworks use different names for their build output:
...
Walrus Sites does not support dynamic redirects. Unlike traditional web servers or hosting platforms that can intercept requests and resolve routes at runtime, Walrus Sites serves static files directly. There is no server-side logic to handle URL resolution.
...
The key is the URL path a visitor or tool requests, and the value is the location of the markdown file in your deployed build.
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/sites/getting-started/publishing-your-first-site.mdx>)

[PreviousInstall the Site Builder](</docs/sites/getting-started/installing-the-site-builder>)[NextSite Builder Reference](</docs/sites/getting-started/using-the-site-builder>)
