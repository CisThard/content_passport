<!-- Source: https://docs.wal.app/docs/sites/ci-cd/other-ci-cd-platforms -->
...
The official [Deploy Walrus Site action](<https://github.com/MystenLabs/walrus-sites-github-actions>) targets GitHub Actions only. On any other CI/CD platform, such as GitLab CI/CD, CircleCI, or Bitbucket Pipelines, you drive deployment directly through the [`site-builder` CLI](</docs/sites/getting-started/using-the-site-builder>). Because `site-builder` is a standard Linux binary, the deployment steps are identical on every platform. Only the pipeline syntax differs.
  ...
  * A Sui address funded with SUI (gas fees) and WAL (storage). See [Preparing Deployment Credentials](</docs/sites/ci-cd/preparing-deployment-credentials>).
  * The private key in `base64WithFlag` format and the corresponding Sui address. See [Exporting your private key](</docs/sites/ci-cd/preparing-deployment-credentials#exporting-your-private-key>).
  * A CI/CD platform with access to a Linux runner.


## How platform-based deployment worksâ
...
## Storing credentials on your platformâ
...
---|---  
GitLab| Settings â CI/CD â Variables (set visibility to **Masked**)  
CircleCI| Project Settings â Environment Variables, or a shared **Context**  
Bitbucket| Repository settings â Repository variables (enable **Secured**)  
...
## Understanding `ws-resources.json` in pipelinesâ
...
Store `SUI_KEYSTORE` and `SUI_ADDRESS` as masked CI/CD variables under **Settings** â **CI/CD** â **Variables**.
...
Store `SUI_KEYSTORE` and `SUI_ADDRESS` as environment variables under **Project Settings** â **Environment Variables** , or in a shared [Context](<https://circleci.com/docs/contexts/>) if multiple projects share the same deployment address.
...
Store `SUI_KEYSTORE` and `SUI_ADDRESS` under **Repository settings** â **Repository variables** with **Secured** enabled.
...
## Adapting to any other platformâ
...
The deploy script itself does not change between platforms. Copy the 5-step script from The deploy script section and inline it into your platform's job definition using that platform's multiline script syntax.

[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/sites/ci-cd/other-ci-cd-platforms.mdx>)

[PreviousGithub Actions Workflow](</docs/sites/ci-cd/github-actions-workflow>)[NextSite Data Authentication](</docs/sites/security/site-data-authentication>)
