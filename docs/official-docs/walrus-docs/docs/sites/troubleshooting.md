<!-- Source: https://docs.wal.app/docs/sites/troubleshooting -->

* [](</>)
  * Troubleshooting Walrus Sites
...
#### `site-builder: command not found`â
...
#### `No such file or directory: sites-config.yaml`â
...
#### `could not retrieve enough confirmations to certify the blob`â
...
#### `InsufficientCoinBalance` or `GasBudgetTooLow`â
...
#### `Error: The wallet must own the site object to update it`â
...
#### Deploy publishes a new site instead of updatingâ
...
#### Site does not load on `wal.app`â
...
#### SuiNS name does not resolve to the correct siteâ
...
#### Pages return a 404 error on direct URL accessâ
...
#### HTTP 503 on route resolution with Content-Security-Policy headersâ
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/sites/troubleshooting.mdx>)

[PreviousKnown Restrictions](</docs/sites/known-restrictions>)
