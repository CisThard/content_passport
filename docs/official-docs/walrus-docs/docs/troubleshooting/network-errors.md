<!-- Source: https://docs.wal.app/docs/troubleshooting/network-errors -->
...
#### `unable to parse the client config file`â
...
#### `the specified Walrus system object does not exist`â
...
#### `Sui RPC url is not specified as a CLI argument or in the client configuration, and no valid Sui wallet was provided`â
...
#### `cannot connect to Sui RPC nodes at URLS`â
...
## Storage duration errorsâ

Errors in this section occur when specifying how long to store a blob.

#### `either epochs or earliest_expiry_time or end_epoch must be provided`â
...
#### `exactly one of epochs, earliest-expiry-time, or end-epoch must be specified`â
...
#### `expiry time is too far in the future`â
...
#### `end_epoch must be greater than the current epoch`â
...
#### `earliest_expiry_time must be greater than the current epoch start time and the current time`â
...
#### `deletable blobs cannot be shared`â
...
#### `exactly one of objectIds, all, or allExpired must be specified`â
...
#### `exactly one of nodeId, nodeUrl, committee, or activeSet must be specified`â
...
#### `node URL URL not found in active set`â
...
#### `unrecognised trace exporter VALUE`â
...
#### `The object ID of an exchange object must be specified`â
...
#### `you seem to be using a numeric value in decimal format corresponding to a Walrus blob ID`â
...
#### `the provided blob ID is invalid`â
...
#### `the blob has not been registered or has already expired`â
...
#### `Insufficient SUI for gas` or `Insufficient WAL for storage fees`â
...
#### `there is enough balance to cover the requested amount but cannot be achieved with less than the maximum number of coins allowed`â
...
#### Publisher cannot reach storage nodesâ
...
#### Aggregator cannot fetch enough sliversâ
...
#### Invalid signature from a storage nodeâ
...
#### Out of memory during encodingâ
...
#### `insufficient balance` despite holding WALâ
...
[Edit this page](<https://github.com/MystenLabs/walrus/tree/main/docs/site/../content/troubleshooting/network-errors.mdx>)

[PreviousTroubleshooting](</docs/troubleshooting>)[NextError Handling](</docs/troubleshooting/error-handling>)
