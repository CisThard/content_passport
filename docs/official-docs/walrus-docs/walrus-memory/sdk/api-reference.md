<!-- Source: https://docs.wal.app/walrus-memory/sdk/api-reference -->
...
### `remember(text, namespace?): Promise<RememberAcceptedResult>`â
...
### `rememberAndWait(text, namespace?, opts?): Promise<RememberResult>`â
...
### `waitForRememberJob(jobId, opts?): Promise<RememberResult>`â

Poll a previously accepted remember job until it reaches `done` or `failed`.

### `rememberBulk(items): Promise<RememberBulkAcceptedResult>`â
...
### `rememberBulkAndWait(items, opts?): Promise<RememberBulkResult>`â

Submit a bulk remember request and wait until every job reaches a terminal state.

### `recall(params): Promise<RecallResult>`â
...
### `analyze(text, namespace?): Promise<AnalyzeResult>`â
...
### `restore(namespace, limit?): Promise<RestoreResult>`â
...
### `compatibility(): Promise<RelayerVersionMetadata>`â
...
### `getPublicKeyHex(): Promise<string>`â
...
### `rememberManual(text, namespace?): Promise<RememberManualResult>`â

Embed locally, Seal encrypt locally, send encrypted payload + vector to relayer for Walrus upload and vector registration.

### `recallManual(query, limit?, namespace?): Promise<RecallManualResult>`â
...
`minRelevance`| `0.3`| Minimum similarity score (0â1) to include a memory  
...
[PreviousReference](</walrus-memory/openclaw/reference>)[NextPython SDK API Reference](</walrus-memory/python-sdk/api-reference>)
