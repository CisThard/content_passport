<!-- Source: https://docs.wal.app/walrus-memory/mcp/reference -->
...
`limit`| integer (1â100)| no| Max memories to return. Default `10`.  
...
`limit`| integer (1â500)| no| Max memories to re-index. Default `10`.  
...
`--namespace <name>` (alias `--ns`)| `MEMWAL_NAMESPACE`| Default memory namespace injected into memory tool calls that omit one. See Default namespace.  
...
`memwal_restore` still lists `namespace` as **required** in its tool schema, so agents normally pass one explicitly. A configured default is only used as a fallback if the agent calls `memwal_restore` without a namespace.
...
### Tools aren't visible to the agentâ
...
### `memwal_login` URL expires before approvalâ
...
### Recall returns "no matching memories found" right after a rememberâ

`memwal_remember` enqueues an async upload to Walrus. Embedding generation, Seal encryption, blob upload, and DB indexing typically take 5â15 seconds. Wait, then retry the recall.

### 401 unauthorized from the relayerâ
...
[PreviousHow It Works](</walrus-memory/mcp/how-it-works>)[NextSmart Contract Overview](</walrus-memory/contract/overview>)
