<!-- Source: https://docs.wal.app/walrus-memory/fundamentals/architecture/data-flow-security-model -->
...
Every protected API call goes through Ed25519 signature verification:
  ...
  4. The relayer resolves the public key to a `MemWalAccount` using the priority chain: cache â signed account header/config fallback â onchain registry scan
...
[PreviousHow Storage Works](</walrus-memory/fundamentals/architecture/how-storage-works>)[NextPermanent Registry Design Intent](</walrus-memory/architecture/permanent-registry-design>)
