<!-- Source: https://docs.wal.app/walrus-memory/python-sdk/api-reference -->
...
`env`| `str`| No| | Hosted preset: `staging` for testing or `prod` for production. Unknown â `ValueError`  
...
### `remember(text, namespace=None) -> RememberAcceptedResult`â
...
### `remember_and_wait(text, namespace=None, poll_interval_ms=1500, timeout_ms=60_000) -> RememberResult`â
...
### `wait_for_remember_job(job_id, poll_interval_ms=1500, timeout_ms=60_000) -> RememberResult`â
...
### `remember_bulk_async(items) -> RememberBulkAcceptedResult`â
...
### `get_remember_bulk_status(job_ids) -> RememberBulkStatusResult`â

One `RememberBulkStatusItem(job_id, status, blob_id?, error?)` per requested job.

### `wait_for_remember_jobs(job_ids, opts=None) -> RememberBulkResult`â
...
### `remember_bulk_and_wait(items, opts=None) -> RememberBulkResult`â

`remember_bulk_async` \+ `wait_for_remember_jobs` in one call.

### `recall(RecallParams(...)) -> RecallResult`â

Search memories matching a natural-language query, scoped to `owner + namespace`. When `max_distance` is set, the client drops weak matches where `distance >= max_distance`.
...
### `analyze(text, namespace=None) -> AnalyzeResult`â
...
### `ask(question, limit=5, namespace=None) -> AskResult`â
...
### `restore(namespace, limit=10) -> RestoreResult`â
...
Method| Description  
---|---  
`remember_manual(RememberManualOptions)`| Register a pre-uploaded blob with a pre-computed vector â `RememberManualResult`  
`recall_manual(RecallManualOptions)`| Search with a pre-computed query vector â `RecallManualResult` (blob_id + distance only)  
`embed(text)`| Embedding vector for text, no storage â `EmbedResult(vector)`  
...
---|---  
`delegate_key_to_sui_address(private_key_hex)`| Derive the `0x`-prefixed Sui address, `blake2b-256(0x00 â pubkey)`  
...
[PreviousTypeScript SDK API Reference](</walrus-memory/sdk/api-reference>)[NextConfiguration](</walrus-memory/reference/configuration>)
