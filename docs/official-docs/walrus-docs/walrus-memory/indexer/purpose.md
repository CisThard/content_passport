<!-- Source: https://docs.wal.app/walrus-memory/indexer/purpose -->
...
The indexer is a standalone Rust service (`services/indexer`) that:
  ...
  4. Inserts `account_id â owner` mappings into the `accounts` table
  5. Stores its event cursor in `indexer_state` so it can resume after restarts


## Auth resolution flowâ
...
[PreviousOwnership and Permissions](</walrus-memory/contract/ownership-and-permissions>)[NextOnchain Events](</walrus-memory/indexer/onchain-events>)
