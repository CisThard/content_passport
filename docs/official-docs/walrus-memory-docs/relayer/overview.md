...
For self-hosted deployments, *all* of these limits and quotas can be fully configured through environment variables. See [Self-Hosting](self-hosting.md) for configuration details.
...
> **Note:** The current relayer only supports a single active package ID at a time. If you deploy a separate Walrus Memory contract, you need to run a separate relayer instance with its own database.
...
If you need to minimize this trust, you can [self-host](self-hosting.md) the relayer, run the [TEE deployment pattern](nautilus-tee.md), or use the [manual client flow](../sdk/usage/memwal-manual.md) to handle encryption and embedding entirely on the client side. See [Trust and Security Model](../fundamentals/architecture/data-flow-security-model.md) for the full breakdown.
