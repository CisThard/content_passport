...
If you want the default relayer-handled SDK flow while reducing trust in the host operator, see the [TEE Deployment Pattern](nautilus-tee.md).
...
See [Database Sync](../indexer/database-sync.md) for the full schema.
...
- Use `LOG_FORMAT=json` in production and see [Observability](observability.md) for dashboards and alerts
...
- [TEE Deployment Pattern](nautilus-tee.md)
- [Relayer API](api-reference.md)
