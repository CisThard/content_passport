...
- Choose a relayer:
  - Use the [managed relayer](../relayer/public-relayer.md), selected with the `env` preset
...
Prefer a synchronous style? Swap `MemWal` for `MemWalSync` and drop the `await`s, see [Usage](usage.md).

## Next steps

- [Usage](usage.md), async vs sync, namespace rules, manual methods, and middleware
- [API Reference](api-reference.md), full method signatures and result types
- [Changelog](changelog.md), release history for `memwal`
