...
This page documents every tool, flag, environment variable, and transport route the Walrus Memory MCP package exposes. For a guided walkthrough, start with the [Quick Start](quick-start.md).
...
> **Warning:** The onchain delegate key registration is **not** revoked by `memwal_logout`, only the local file is wiped. Visit the [Walrus Memory dashboard](https://memory.walrus.xyz) to remove the delegate key from your account.

> **Note:** Both session tools (`memwal_login`, `memwal_logout`) are intercepted locally by the stdio package and never reach the relayer. They read and write files on the client machine only.
...
1. **Explicit per-call `namespace`**, a non-empty `namespace` in the tool call is used as-is. The configured default never overrides it.
2. **`--namespace` CLI flag** (alias `--ns`).
3. **`MEMWAL_NAMESPACE` environment variable**.
4. **Unset**, the call is forwarded without a `namespace` and the relayer applies its own `"default"` namespace.

CLI wins over the environment variable when both are set, matching every other flag in the table above.

> **Note:** `memwal_restore` still lists `namespace` as **required** in its tool schema, so agents normally pass one explicitly. A configured default is only used as a fallback if the agent calls `memwal_restore` without a namespace.
...
> **Warning:** Treat the delegate private key in this file like an API key. Anyone who gets it can act as this MCP client until the delegate is revoked.
...
> **Warning:** The bearer token is a long-lived credential equivalent to an API key. **Never commit MCP configs with a real `Authorization` header to source control.** Treat it like any other secret.
...
See [Environment Variables](../reference/environment-variables.md) for the full list including Seal, Walrus, embeddings, and database settings.
...
This is the expected first-run state. Credentials are missing. Ask the agent to call `memwal_login`, approve the browser flow, then retry the original memory action.

### Tools are missing after `memwal_login`

The browser flow succeeded but the client may need a full restart to pick up the new credentials. Quit and reopen the MCP client.

### MCP server won't start

- Confirm Node.js 20+ is installed: `node -v`
- Confirm the package resolves: `npx -y @mysten-incubation/memwal-mcp --help`
- Check the client config file path matches your client's expected location
- Restart the client fully after editing the config

### `memwal_login` returns a URL but nothing happens

The browser should open automatically. If it doesn't, copy the URL from the tool response and paste it into a browser manually. The local listener stays alive for 5 minutes.

### Memory tools return errors after login

- Check that the relayer URL in `~/.memwal/credentials.json` is reachable
- Verify the account ID matches the delegate key
- Run `memwal_login` again to refresh credentials if needed
