...
> **Warning:** Run this in a real terminal (with a TTY). The login command opens a browser and waits for your wallet approval. If you wrap it in a non-interactive shell, the browser won't pop and the flow exits silently.
...
Those values come from `~/.memwal/credentials.json` after a successful login. See [Reference](reference.md#streamable-http) for the full config shape and security notes.
...
Your client config doesn't change, the package reads the saved environment from `~/.memwal/credentials.json` on each run. See [Environment presets](reference.md#environment-presets) for all four shortcuts.
...
- **[Reference](reference.md)** - All six tools with parameters, CLI flags, and transport routes
- **[How It Works](how-it-works.md)** - Auth-required mode, local credentials, and bridge behavior
- **Streamable HTTP transport** - Skip `npx`, point your client at the relayer URL directly
- **[Self-Hosting](../relayer/self-hosting.md)** - Run your own relayer and route MCP traffic through it
- **[Environment Variables](../reference/environment-variables.md)** - Full list of relayer + sidecar settings (Seal, Walrus, sessions)
