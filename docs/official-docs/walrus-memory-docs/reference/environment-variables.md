...
Use this page when you run your own relayer. For setup steps and deployment context, see [Self-Hosting](../relayer/self-hosting.md).

Environment variables documented here are public relayer contract items. Renaming, removing, or changing their meaning follows the deprecation process in [Versioning and Compatibility](../relayer/versioning-and-compatibility.md).
...
| `MEMWAL_PACKAGE_ID` | Sui package ID. See [Contract Overview](../contract/overview.md) |
| `MEMWAL_REGISTRY_ID` | Onchain registry object ID. See [Contract Overview](../contract/overview.md) |
...
- For network-specific `MEMWAL_PACKAGE_ID` and `MEMWAL_REGISTRY_ID` values, see [Contract Overview](../contract/overview.md).
