...
When adding a preview to your app, use [`PreviewView`](</reference/kotlin/androidx/camera/view/PreviewView>), which is a `View` that can be cropped, scaled, and rotated for proper display.
...
Implementing a preview for CameraX using `PreviewView` involves the following steps, which are covered in later sections:
  ...
  1. Optionally configure a [`CameraXConfig.Provider`](</reference/kotlin/androidx/camera/core/CameraXConfig.Provider>).
  2. Add a `PreviewView` to your layout.
  3. Request a [`ProcessCameraProvider`](</reference/kotlin/androidx/camera/lifecycle/ProcessCameraProvider>).
...
Using `PreviewView` has some limitations. When using `PreviewView`, you can't do any of the following things:
  ...
  * Create a `SurfaceTexture` to set on `TextureView` and [`Preview.SurfaceProvider`](</reference/androidx/camera/core/Preview.SurfaceProvider>).
...
CameraX `PreviewView` provides some additional APIs to configure properties such as:
  ...
  * The [implementation mode](</reference/androidx/camera/view/PreviewView.ImplementationMode>) for rendering preview streams.
